...
class VueBooker:
    ...
    def get_email(self):
        self.header("Email Required")
        print("Vue requires an email address for bookings.")
    ...
    def select_cinema(self):
        ...
        # Create a temporary client to fetch cinemas (no site_id)
        temp_api = Cinema.Vue()
        try:
            sites_data = temp_api.cinemas()
            self.log(f"Sites data: {sites_data}", "debug")
        ...
        # Parse Vue's nested structure: result -> list of {alpha, cinemas: [...]}
        all_cinemas = []
        raw_result = sites_data.get('result', [])
        if isinstance(raw_result, list):
            for group in raw_result:
                cinemas = group.get('cinemas', [])
                if isinstance(cinemas, list):
                    all_cinemas.extend(cinemas)
        else:
            if isinstance(sites_data, list):
                all_cinemas = sites_data
            elif isinstance(sites_data.get('sites'), list):
                all_cinemas = sites_data['sites']
            else:
                self.log("Could not parse cinema list", "error")
                return False

        if not all_cinemas:
            self.log("No cinemas found", "error")
            return False

        matches = []
        for c in all_cinemas:
            name = c.get('cinemaName', '')
            full = c.get('fullName', '')
            combined = f"{name} {full}".lower()
            if search.lower() in combined:
                matches.append(c)

        if not matches:
            print(f"\n\033[93mNo cinema found matching '{search}'. Showing available cinemas (first 30):\033[0m")
            for i, c in enumerate(all_cinemas[:30], 1):
                name = c.get('cinemaName', 'Unknown')
                full = c.get('fullName', '')
                print(f"  {i}. {name} - {full}")
            if len(all_cinemas) > 30:
                print(f"  ... and {len(all_cinemas)-30} more")
            choice = self.get_valid_input("Enter the number of your cinema (or 0 to cancel): ")
            try:
                idx = int(choice) - 1
                if idx >= 0 and idx < len(all_cinemas):
                    selected = all_cinemas[idx]
                else:
                    return False
            except:
                return False
            ...
            print(f"\n\033[93mFound {len(matches)} matching cinemas:\033[0m")
        ...
        self.site_id = selected.get('cinemaId')
        if not self.site_id:
            self.log("No cinemaId found for selected cinema", "error")
            return False
        self.cinema = selected
        # Create the main API client WITHOUT site_id (VueClient doesn't accept it)
        self.api = Cinema.Vue()
        name = selected.get('cinemaName', 'Unknown')
        print(f"✓ Selected: {name}")
        time.sleep(0.5)
        return True

    # ---- Everything below is unchanged except we use self.site_id in method calls ----
    ...
    def select_film(self):
        ...
        try:
            films_data = self.api.films(self.site_id)   # pass site_id
    ...
    def select_showtime(self):
        ...
        try:
            dates_data = self.api.dates(self.film_id, self.site_id)   # pass site_id
        ...
        try:
            showtimes_data = self.api.showtimes(self.film_id, selected_date, self.site_id)   # pass site_id
    ...
    def get_available(self):
        if not self.showtime_id:
            return []
        try:
            seat_data = self.api.seats(self.showtime_id)   # no site_id needed
            ...
            if not layout_id:
                ...
                return []
            layout = self.api.seat_layout(layout_id)   # no site_id needed
            available = []
    ...
    def get_ticket_candidates(self):
        if not self.showtime_id:
            return []
        try:
            prices = self.api.tickets(self.showtime_id)   # no site_id needed
            valid = []
    ...
    def book_chunk(self, ids, labels):
        if not self.email:
            self.log("No email set for Vue booking", "error")
            return None
        ...
        for tt in candidates:
            try:
                ...
                order = self.api.create_order(email=self.email)   # Vue requires email
