...
class VueClient(BaseClient):
    ...
    def _auth(self, path: str):
        ...
        if isinstance(payload, dict) and payload.get("responseCode") not in (
            None,
            0,
            "0",
        ):
    ...
    def request(
        self, method: str, path: str, *, params=None, json_body=None, retry=True
    ):
        ...
        if isinstance(payload, dict):
            if payload.get("responseCode") not in (None, 0, "0") or payload.get(
                "errorMessage"
            ):
    ...
    def cinemas(self, *, refresh: bool = False):
        return self._cached(
            ("raw-cinemas",), lambda: self.request("GET", "showings/cinemas"), refresh
        )
    ...
    def films(self, cinema_id: str, *, refresh: bool = False):
        return self._cached(
            ("raw-films", cinema_id),
            lambda: self.request(
                "GET", "showings/films", params={"cinemaId": cinema_id}
            ),
            refresh,
        )
    ...
    def showtimes(self, cinema_id: str, film_id: str, date: str):
        return self.request(
            "GET",
            f"showings/cinemas/{cinema_id}/films/{film_id}/showingGroups",
            params={"showingDate": date},
        )

    def showing(self, cinema_id: str, session_id: str):
        return self.request(
            "GET", f"showings/cinemas/{cinema_id}/showings/{session_id}"
        )
    ...
    def seats(self, cinema_id: str, session_id: str, order_id: str | None = None):
        ...
        return self.request(
            "GET", f"booking/Session/{cinema_id}/{session_id}/seats", params=params
        )
    ...
    def make_order(
        ...
        seats: Sequence[Mapping[str, int]],
    ):
    ...
    def locations(self, *, refresh=False):
        def load():
            ...
            for item in dictionaries(
                self.cinemas(refresh=refresh), ("result", "cinemas", "items")
            ):
                identifier = str(item.get("cinemaId") or item.get("id") or "")
                name = str(item.get("cinemaName") or item.get("name") or "")
                if identifier and name:
                    result.append(
                        CinemaLocation(
                            ...
                            id=identifier,
                            name=name,
                            city=str(item.get("city") or item.get("town") or ""),
    ...
    def _high_films(self, cinema, *, refresh=False):
        result = []
        for item in dictionaries(
            self.films(cinema.id, refresh=refresh), ("result", "films", "items")
        ):
            identifier = str(item.get("filmId") or item.get("id") or "")
            title = str(
                item.get("filmTitle") or item.get("title") or item.get("name") or ""
            )
            if identifier and title and item.get("hasSessions", True):
                result.append(
                    Film(
                        raw=item,
                        id=identifier,
                        title=title,
                        runtime=item.get("runningTime"),
                        rating=str((item.get("certificate") or {}).get("name") or ""),
                        cinema=cinema,
                        _client=self,
                    )
                )
    ...
    def _high_dates(self, film, *, refresh=False):
        payload = self._cached(
            ...
            lambda: self.dates(film.cinema.id, film.id),
            refresh,
        )
        values = payload.get("result") or payload.get("dates") or []
        return [
            str(x.get("date") or x.get("showingDate") if isinstance(x, dict) else x)
            for x in values
        ]
    ...
    def _high_showtimes(self, film, date, *, refresh=False):
        payload = self._cached(
            ...
            lambda: self.showtimes(film.cinema.id, film.id, date),
            refresh,
        )
        result = []
        for group in payload.get("result", []):
            for item in group.get("sessions", []):
                identifier = str(item.get("sessionId") or "")
                start = str(
                    item.get("startTime")
                    or item.get("showingTime")
                    or item.get("start")
                    or ""
                )
                attrs = [
                    str(x.get("shortName") or x.get("name") or "")
                    for x in item.get("attributes", [])
                ]
                if identifier:
                    result.append(
                        Showtime(
                            raw=item,
                            id=identifier,
                            advertised_start=provider_datetime(start),
                            film_start=provider_datetime(item.get("filmStartTime")),
                            ends_at=provider_datetime(item.get("endTime")),
                            screen=str(item.get("screen") or ""),
                            experience=", ".join(filter(None, attrs)),
                            cinema=film.cinema,
                            film=film,
                            _client=self,
                        )
                ...
                if label:
                    result.append(
                        Seat(
                            ...
                            row=match_row,
                            number=number,
                            position=int(item.get("columnIndex") or 0),
                            available=int(item.get("seatStatus") or 0) == 0,
                            type=str(item.get("seatType") or "standard"),
                        )
                    )
        return result

    def _high_tickets(self, showing, *, unrestricted_only=True):
        payload = self.tickets(showing.cinema.id, showing.id)
        result = []
        for item in dictionaries(payload, ("result", "tickets", "items")):
            identifier = str(
                item.get("code") or item.get("ticketTypeId") or item.get("id") or ""
            )
            name = str(item.get("name") or item.get("description") or identifier)
            cents = item.get("priceInCents") or item.get("price")
            ticket = TicketType(
                raw=item,
                id=identifier,
                name=name,
                price=Decimal(str(cents)) / 100 if cents is not None else None,
                restrictions=tuple(
                    str(value) for value in item.get("restrictions", [])
                ),
                member_only=bool(item.get("memberOnly")),
                subscription_id=str(item.get("subscriptionId"))
                if item.get("subscriptionId")
                else None,
                area_category_id=str(item.get("areaCategoryCode") or "") or None,
            )
    ...
    def _high_book(self, showing, seats, ticket, *, email=None):
        if not email:
            raise ValueError("email is required for Vue bookings")
        selected = showing.seats(seats)
        ticket_type = showing.ticket(ticket)
        coordinates = [
            {
                "areaNumber": s.raw["areaNumber"],
                "rowIndex": s.raw["rowIndex"],
                "columnIndex": s.raw["columnIndex"],
            }
            for s in selected
        ]
        ticket_data = {
            "areaCategoryCode": ticket_type.raw.get("areaCategoryCode")
            or selected[0].raw.get("areaCategoryCode"),
            "code": ticket_type.id,
            "priceInCents": ticket_type.raw.get("priceInCents")
            or int((ticket_type.price or 0) * 100),
        }
        body = self.make_order(
            showing.cinema.id, showing.id, email, ticket_data, coordinates
        )
        created = self.create_order(body)
        order_id = str((created.get("result") or created).get("orderSessionId") or "")
        if not order_id:
            raise CinemaError("Vue did not return an orderSessionId")
        return Booking(
            raw=created,
            order_id=order_id,
