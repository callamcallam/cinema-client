...
class Session:
    ...
    def get(self, url, **kw): self.calls.append(("GET", url, kw)); return next(self.responses)
    def post(self, url, **kw): self.calls.append(("POST", url, kw)); return next(self.responses)
    def request(self, method, url, **kw): self.calls.append((method, url, kw)); return next(self.responses)
    def close(self): pass
...
def test_odeon_cancel_is_delete_and_accepts_204():
    ...
    assert session.calls[0][0:2] == ("DELETE", "https://example.test/ocapi/v1/orders/abc")
...
def test_vue_order_shape():
    body = Cinema.Vue.make_order("c", "s", "e@example.com", {"areaCategoryCode": "a", "code": "t", "priceInCents": 10}, [{"areaNumber": 1, "rowIndex": 2, "columnIndex": 3}])
